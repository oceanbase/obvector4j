...
public class HybridSearchTest {
     ...
     */
    @Test
    public void testHybridTextVectorSearch() throws Throwable {
            Class.forName("com.oceanbase.jdbc.Driver");
            
            // Use unique table name to avoid conflicts
            String tb_name = "HYBRID_TEXT_VECTOR_TEST_" + System.currentTimeMillis();
            ...
            // Create full-text indexes (may fail if disk space is low, but test can continue)
            try {
                ob.createFulltextIndex(tb_name, "ft_title", "title");
            } catch (Throwable e) {
                System.out.println("Warning: Failed to create ft_title index: " + e.getMessage());
            }
            try {
                ob.createFulltextIndex(tb_name, "ft_content", "content");
            } catch (Throwable e) {
                System.out.println("Warning: Failed to create ft_content index: " + e.getMessage());
            }
            ...
            // Check if fulltext indexes exist before testing
            boolean hasTitleIndex = ob.hasIndex(tb_name, "ft_title");
            boolean hasContentIndex = ob.hasIndex(tb_name, "ft_content");
            
            if (!hasTitleIndex || !hasContentIndex) {
                System.out.println("Warning: Fulltext indexes not available, skipping text vector search tests");
                // Still test vector search only
                float[] queryVector = generateRandomVector(128);
                ArrayList<HashMap<String, Sqlizable>> vectorResults = ob.query(
                    tb_name, "embedding", "cosine", queryVector, 3,
                    new String[] {"title", "content"},
                    new DataType[] {DataType.STRING, DataType.STRING},
                    null
                );
                assertNotNull("Vector search results should not be null", vectorResults);
                return; // Skip text vector search tests
            }
     ...
     */
    @Test
    public void testHybridScalarVectorSearch() throws Throwable {
            Class.forName("com.oceanbase.jdbc.Driver");
            
            // Use unique table name to avoid conflicts
            String tb_name = "HYBRID_SCALAR_VECTOR_TEST_" + System.currentTimeMillis();
            ObVecClient ob = new ObVecClient(URI, USER, PASSWORD);
     ...
     */
    @Test
    public void testFilterAPI() throws Throwable {
            Class.forName("com.oceanbase.jdbc.Driver");
            
            // Use unique table name to avoid conflicts
            String tb_name = "HYBRID_SCALAR_VECTOR_FILTER_TEST_" + System.currentTimeMillis();
            ObVecClient ob = new ObVecClient(URI, USER, PASSWORD);
     ...
     */
    @Test
    public void testFilterAPIWithTextVectorSearch() throws Throwable {
            Class.forName("com.oceanbase.jdbc.Driver");
            
            // Use unique table name to avoid conflicts
            String tb_name = "HYBRID_TEXT_VECTOR_FILTER_TEST_" + System.currentTimeMillis();
            ObVecClient ob = new ObVecClient(URI, USER, PASSWORD);
            ...
            try {
                ...
                // Check if fulltext index exists before testing
                boolean hasTitleIndex = ob.hasIndex(tb_name, "ft_title");
                
                if (!hasTitleIndex) {
                    System.out.println("Warning: Fulltext index not available, testing scalar vector search instead");
                    // Still test scalar vector search with filter
                    float[] queryVector = generateRandomVector(128);
                    Filter filter = FilterBuilder.and(
                        FilterBuilder.key("category_id").isEqualTo(1),
                        FilterBuilder.key("price").isLessThanOrEqualTo(250.0)
                    );
                    ArrayList<HashMap<String, Sqlizable>> scalarResults = ob.scalarVectorSearch()
                        .table(tb_name)
                        .queryVector(queryVector)
                        .filter(filter)
                        .topk(10)
                        ...
                        .search();
                    assertNotNull("Scalar vector search results should not be null", scalarResults);
                    for (HashMap<String, Sqlizable> row : scalarResults) {
                        int catId = Integer.parseInt(row.get("category_id").toString());
                        double priceVal = Double.parseDouble(row.get("price").toString());
                        assertEquals("category_id should be 1", 1, catId);
                        assertTrue("price should be <= 250", priceVal <= 250.0);
                    }
                    return; // Skip text vector search test
                }
     ...
     */
    @Test
    public void testOriginalHybridSearchAPI() throws Throwable {
            Class.forName("com.oceanbase.jdbc.Driver");
            
            // Use unique table name to avoid conflicts
            String tb_name = "HYBRID_TEXT_VECTOR_TEST_ORIGINAL_" + System.currentTimeMillis();
            ObVecClient ob = new ObVecClient(URI, USER, PASSWORD);
            ...
            try {
                ...
                // Create full-text index (may fail if disk space is low, but test can continue)
                try {
                    ob.createFulltextIndex(tb_name, "ft_title", "title");
                } catch (Throwable e) {
                    System.out.println("Warning: Failed to create ft_title index: " + e.getMessage());
     ...
     */
    @Test
    public void testQuickStartFullTextVectorSearch() throws Throwable {
            Class.forName("com.oceanbase.jdbc.Driver");
            
            // Use unique table name to avoid conflicts
            String tb_name = "HYBRID_TEXT_VECTOR_QUICKSTART_" + System.currentTimeMillis();
            ObVecClient ob = new ObVecClient(URI, USER, PASSWORD);
     ...
     */
    @Test
    public void testQuickStartScalarVectorSearch() throws Throwable {
            Class.forName("com.oceanbase.jdbc.Driver");
            
            // Use unique table name to avoid conflicts
            String tb_name = "HYBRID_SCALAR_VECTOR_QUICKSTART_" + System.currentTimeMillis();
            ObVecClient ob = new ObVecClient(URI, USER, PASSWORD);
