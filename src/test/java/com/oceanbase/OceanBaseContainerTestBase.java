...
public class OceanBaseContainerTestBase {
     ...
     * Should be called in @BeforeClass method (JUnit 4) or @BeforeAll (JUnit 5).
     */
    ...
    public static void initContainer() throws Throwable {
        ...
        if (uriFromEnv == null) {
            container = new OceanBaseCEContainer(DockerImageName.parse(OCEANBASE_IMAGE))
                    .withEnv("MODE", "slim")
                    .withEnv("OB_DATAFILE_SIZE", "4G") // Increased to 8G to avoid disk space issues
     ...
     * Should be called in @AfterClass method (JUnit 4) or @AfterAll (JUnit 5).
     */
